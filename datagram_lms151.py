...
Sick561Datagram = collections.namedtuple("sick151_datagram", ["TypeOfCommand",
                                                               "Command",
                                                               "VersionNumber",
                                                               "DeviceNumber",
                                                               "SerialNumber",
...
def decode_datagram(datagram):
    ...
    if header['TypeOfCommand'] != '\x02sRA':
        return None
    ...
    header['ScanningFrequency'] = parse_number(items[16])
    header['MeasurementFrequency'] = parse_number(items[17])
    header['ScalingFactor'] = parse_number(items[23])
    header['ScalingOffset'] = parse_number(items[24])
    header['StartingAngle'] = parse_number(items[25])
    header['AngularStepWidth'] = parse_number(items[26])
    header['NumberOfData'] = parse_number(items[27])
    header['Data'] = [parse_number(x) / 1000 for x in items[28:28+header['NumberOfData']]]
